<x-admin-layout title="{{ __('admin.page_users') }}">
    <div class="flex items-center justify-between">
        <h1 class="text-2xl font-bold text-gray-900">{{ __('admin.page_users') }}</h1>
    ...
    <form method="GET" action="{{ route('admin.users') }}" class="mt-4">
        <div class="flex gap-2">
            ...
            <button type="submit" class="rounded-lg bg-gray-100 px-4 py-2 text-sm font-medium text-gray-700 transition hover:bg-gray-200">{{ __('admin.btn_search') }}</button>
        </div>
    </form>
            ...
            <table class="min-w-full divide-y divide-gray-200">
                <thead class="bg-gray-50">
                    <tr>
                        ...
                        <th class="px-4 py-3 text-left text-xs font-medium uppercase tracking-wider text-gray-500">{{ __('admin.col_admin') }}</th>
                        <th class="px-4 py-3 text-left text-xs font-medium uppercase tracking-wider text-gray-500">{{ __('admin.col_permissions') }}</th>
                        <th class="px-4 py-3 text-left text-xs font-medium uppercase tracking-wider text-gray-500">{{ __('admin.col_registered') }}</th>
                ...
                <tbody class="divide-y divide-gray-200">
                    ...
                    <tr class="hover:bg-gray-50">
                        <td class="whitespace-nowrap px-4 py-3 text-sm font-medium text-gray-900">
                            ...
                            @if($user->isSuperAdmin())
                                <span class="ml-1 inline-flex items-center rounded-full bg-amber-100 px-2 py-0.5 text-xs font-medium text-amber-800">{{ __('admin.badge_super') }}</span>
                            @endif
                        </td>
                        ...
                        <td class="whitespace-nowrap px-4 py-3 text-sm">
                            ...
                            <form method="POST" action="{{ route('admin.users.permissions', $user) }}" class="flex gap-3">
                                ...
                                <label class="flex items-center gap-1">
                                    ...
                                    <span class="text-xs text-gray-600">{{ __('admin.perm_clients') }}</span>
                                </label>
                                <label class="flex items-center gap-1">
                                    <input type="checkbox" name="can_view_messages" value="1" {{ $user->can_view_messages ? 'checked' : '' }}
                                           class="rounded border-gray-300 text-teal-600 focus:ring-teal-500" onchange="this.form.submit()">
                                    <span class="text-xs text-gray-600">{{ __('admin.perm_messages') }}</span>
                                </label>
                            </form>
                            @else
                                <span class="text-xs text-gray-400">{{ __('admin.all_permissions') }}</span>
                            @endif
                        </td>
                        <td class="whitespace-nowrap px-4 py-3 text-sm text-gray-500">{{ $user->created_at ? $user->created_at->format('d.m.Y') : '-' }}</td>
                        <td class="whitespace-nowrap px-4 py-3 text-sm">
                            @unless($user->isSuperAdmin() || $user->id === auth()->id())
                            <form method="POST" action="{{ route('admin.users.destroy', $user) }}" onsubmit="return confirm('{{ __('admin.confirm_delete_user', ['name' => $user->name]) }}')">
                                @csrf @method('DELETE')
                    ...
                    @empty
                    <tr>
                        <td colspan="6" class="px-4 py-8 text-center text-sm text-gray-500">{{ __('admin.no_users') }}</td>
                    </tr>
                    @endforelse
                </tbody>
            </table>
